...
class ClosedAssessmentRepository {
   ...
   uid=user.uid;
    ...
    for (int i = 0; i < closed.docs.length; i++) {
      if (closed.docs[i].data()['currentStatus'] == 'Closed' &&
          (closed.docs[i].data()['coAssessorUid'] == uid ||
              closed.docs[i].data()['assessorUid'] == uid)) {
        ...
        info['assessorName'] = await assessor(closed.docs[i]['assessorUid']);
        info['assesseeName'] = await assessee(closed.docs[i]['assesseeUid']);
        info['coAssessorName'] =
